...
test('ffi validates memory access arguments', () => {
  withAllocations(common.mustCall((alloc) => {
    ...
    assert.throws(() => ffi.toBuffer(ptr, 4, 0), /The copy must be a boolean/);
    assert.throws(() => ffi.toBuffer(ptr, 4, ''), /The copy must be a boolean/);
    assert.throws(() => ffi.toBuffer(ptr, 4, null), /The copy must be a boolean/);
    assert.throws(() => ffi.toArrayBuffer(ptr, 4, 0), /The copy must be a boolean/);
    assert.throws(() => ffi.toArrayBuffer(ptr, 4, 1), /The copy must be a boolean/);
    assert.throws(() => ffi.toArrayBuffer(ptr, 4, {}), /The copy must be a boolean/);
